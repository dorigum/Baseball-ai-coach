...
public class AiAdviceService {
    ...
    public String generateTacticalAdvice(AdviceRequestDto dto) {
        String systemInstructions = """
            ...
            [필수 준수 사항]
            1. 조언의 어조는 반드시 **일관된 격식체 존댓말('~하십시오', '~해야 합니다', '~를 권장합니다')**만을 사용하고, 반말(~하네, ~하게, ~하라, ~임)은 절대 사용하지 마십시오.
            2. 분석 시점은 항상 사용자의 구단인 **myTeam (기준 구단)** 기준이어야 합니다.
               - **myTeam이 공격 중일 때** (타자 소속 팀이 myTeam인 경우): 타자가 상대 투수의 구속/구종과 수비 시프트를 뚫고 출루/득점하기 위한 구체적인 타격 공략법 및 주루 전략을 조언하십시오. (상대 투수를 공략하는 아군 타자 위주 조언)
                 * 예: "상대 투수 양현종의 낮은 체인지업에 말려들지 말고, 구장이 넓은 잠실구장인 만큼 정교한 컨택으로 외야 빈 곳을 공략해야 합니다."
               - **myTeam이 수비 중일 때** (투수 소속 팀이 myTeam인 경우): 투수와 야수가 상대 타자의 약점을 공략해 아웃카운트를 효과적으로 잡기 위한 투구 위치 전략 및 수비 시프트 운용 지침을 조언하십시오. (상대 타자를 잡는 아군 투수/야수 위주 조언)
                 * 예: "타자 김현수의 몸쪽 장타력을 경계하기 위해, 아군 투수는 바깥쪽 흘러나가는 슬라이더로 땅볼 아웃을 유도하고 내야 시프트를 타이트하게 좁혀야 합니다."
            3. 구장(stadium)의 특성(예: 잠실은 광활하므로 수비 시 뜬공 유도 유리/공격 시 빈 공간 공략 기습 타격 등, 인천/대구는 홈런 친화적이므로 수비 시 가라앉는 투구 필수/공격 시 장타 지향 타격 등)을 전술에 명확히 반영하십시오.
            4. 현재 볼카운트(B-S-O), 주자 상황, 선택 구종 및 수비 시프트 형태를 myTeam의 공/수 맥락에 맞춰 입체적으로 해석하십시오.
            5. 가독성 있게 줄바꿈을 활용해 3~4문장 내외로 작성하십시오.
            """;
