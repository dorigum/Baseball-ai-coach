...
public class AiAdviceService {
    ...
    public String generateTacticalAdvice(AdviceRequestDto dto) {
        String systemInstructions = """
            ...
            7. **[가장 중요]** 전술 조언 분석이 완료되면 반드시 맨 마지막 줄에 개행을 한 번 하고 `[RECOMMENDED_SHIFT: <SHIFT_TYPE>]` 형태로 수비 시프트 코드를 덧붙여 출력하십시오.
               - <SHIFT_TYPE> 종류:
                 * `STANDARD`: 기본 표준 수비
                 * `PULL_LEFT`: 상대 타자가 좌타자이고 당겨치는 성향이 강할 때 내야/외야를 우측(1루/2루간)으로 좁히는 시프트
                 * `PULL_RIGHT`: 상대 타자가 우타자이고 당겨치는 성향이 강할 때 내야/외야를 좌측(3루/유격수간)으로 좁히는 시프트
                 * `BUNT`: 무사 또는 1사 상황에서 주자가 있어 번트 희생타가 예상될 때 1, 3루수가 홈 쪽으로 전진하는 수비
                 * `DEEP`: 홈런이 나오기 쉬운 대구/인천 구장이거나 타자가 극단적 장타자일 때 외야수를 펜스 뒤로 깊게 배치하는 수비
               - myTeam이 공격 중인 상황(타석의 타자가 myTeam 소속)이거나 수비 시프트를 바꿀 필요가 없다면 `STANDARD`로 지정하십시오.
            """;
