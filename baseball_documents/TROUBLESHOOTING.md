...
- **발생일**: 2026-07-24
- **요약**: 백엔드 오프라인 시 작동하는 로컬 룰엔진이, 기준 구단의 공격 상황임에도 수비 팁(피칭 전략)만 뱉던 결함 조치
- **원인**:
  `triggerLocalFallback` 내부 분기가 이닝 초/말에 따른 공격/수비 역할을 체크하지 않고 투수 시점 조언 텍스트만 하드코딩해서 반환하고 있었음.
- **해결**:
  현재 이닝 진행 상황에 따른 `myTeam`의 공/수 역할을 판별하는 조건절을 신설하여, 공격 시에는 '타격/출루 팁', 수비 시에는 '피칭/시프트 팁'으로 분기되어 출력되도록 로컬 폴백 텍스트 연산을 고도화함.
- **상세 기록**: [2026-07-24 개발 로그](project-log/2026-07-24.md)
